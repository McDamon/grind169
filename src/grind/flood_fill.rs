...
impl Solution {
    pub fn flood_fill(image: Vec<Vec<i32>>, sr: i32, sc: i32, color: i32) -> Vec<Vec<i32>> {
        ...
        if sr < image.len() as i32 && sc < image[0].len() as i32 {
            ...
            Self::flood_fill_inner(&mut image_mut, sr, sc, scolor, color);
        }
        image_mut
    }
    ...
    fn flood_fill_inner(image: &mut [Vec<i32>], sr: i32, sc: i32, scolor: i32, color: i32) {
        if image[sr as usize][sc as usize] != color {
            ...
            if sr > 0 && image[(sr - 1) as usize][sc as usize] == scolor {
                Self::flood_fill_inner(image, sr - 1, sc, scolor, color);
            }

            // Left is sr, sc - 1
            if sc > 0 && image[sr as usize][(sc - 1) as usize] == scolor {
                Self::flood_fill_inner(image, sr, sc - 1, scolor, color);
            }
...
#[cfg(test)]
mod tests {
    use crate::grind::flood_fill::Solution;

    #[test]
    fn test_flood_fill_case1() {
        let result = Solution::flood_fill(
    ...
    fn test_flood_fill_case2() {
        let result =
            Solution::flood_fill([[0, 0, 0].to_vec(), [0, 0, 0].to_vec()].to_vec(), 0, 0, 0);
